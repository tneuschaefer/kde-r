...
# kernel_estimator(x, built_in = "gaussian", bandwidth = 1) would also build f1 
# because dnorm is exactly the Gaussian kernel 
...
Now that we have the density of our sample we can simulate new samples using rejection_sampling_factory(), this method takes a density you want to take samples from (sample_density), a proposal distribution and its density and a numeric positive scalar M satisfying sample_density(x) <= M * density(x) for all x as an input and gives a distribution function like runif(n) as an output. The function itself produces samples y from the proposal distribution and samples from a uniform-distribution u and accept/rejects y iff u * M * proposal_density(y) <= sample_density(y). Samples are accepted/rejected with probability of 1/M, therefor it takes M samples to accept one and a small selection of M is preferable. If no suitable M is known the user can give the function an interval that contains a maximum of sample_density/density, the function will try to compute the maximum and uses this as M. Note that this M does not have to be optimal and in general propsal distributions similar to the  the sample_density-distribution produce better samples. Alternatively rejection_sample() also takes an integer n als input and directly gives samples. Note that rejection_sample() uses rejection_sample_factory(). We can generate samples of our estimator as an example:
