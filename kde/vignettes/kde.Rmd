...
```{r, fig.dim=c(5,5)}
g1 <- kernel_estimator(x, kernel = dunif, bandwidth = 1)
g2 <- kernel_estimator(x, kernel = dunif, bandwidth = 0.5)
g3 <- kernel_estimator(x, kernel = dunif, bandwidth = 0.1)
...
h2 <- cross_validation(x, dnorm)
K1 <- kernel_estimator(x, kernel = dnorm, bandwidth = h1)
K2 <- kernel_estimator(x, kernel = dnorm, bandwidth = h2)
...
```{r, fig.dim=c(5,5)}
estimate_sample <- rejection_sampling_factory(K2, rnorm, dnorm, interval = c(-1, 1))
